...
def num_sevens(n):
    ...
    def getlist(n):
        return str(n)
    return sum([1 for num in getlist(n) if num=='7'])
...
def pingpong(n):
    ...
    def getturns(n):
        if n<=6:
            ...
            return sum([1 for i in range(n+1) if '7'in str(i) or (i>0 and i%7==0)])
    def getlist(left,right,n):
        if getturns(n)%2==0:
            return left
        else:
            return right
    return sum([getlist(1,-1,i) for i in range(n)])
